...
async def get_usd_rate():
    try:
        url = "https://api.binance.com/api/v3/ticker/price?symbol=USDRUB"
...
async def create_crypto_invoice(amount_rub):
    try:
        ...
        amount_usd = Decimal(str(amount_rub)) / usd_rate
        amount_usd = amount_usd.quantize(Decimal('0.01'))
        
        logging.info(f"Создаю чек: {amount_rub} RUB = {amount_usd} USD (курс: {usd_rate})")
...
async def purchase_account(call: CallbackQuery):
    ...
    if account.get("status") != "ожидает":
        await call.answer("❌ Этот аккаунт уже продан!", show_alert=True)
        await call.message.delete()
        return
